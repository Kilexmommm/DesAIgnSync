...
| 5 — Vertical slice | DS-018 (+ APIs DS-008/DS-017) | ✅ mergeado | `ReviewOrchestrator` (Chrome MCP → evidencia → DS MCP → firmas → matching → reglas → LLM opcional → `ReviewResult`); `elementEvidenceCollector` read-only (sin `value` ni atributos fuera de allowlist); resolución de página por id con fallback por URL; endpoints `/inspection/review`, `/llm/providers*`, `/profiles`; `LlmProviderRegistry` y `ProfileRegistry`; **Side Panel cableado**: selector de perfil, botón Review y vista auditable (`ReviewView`) con match/confidence, checklist por categoría, interpretación IA marcada y warnings |
